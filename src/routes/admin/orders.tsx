...
function OrdersPage() {
  ...
  async function load() {
    setLoading(true);
    const res = await adminOrdersApi.list({ page, page_size: 15, search: search || undefined, status: statusFilter || undefined });
    setOrders(res.data || []);
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2 flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && load()}
            placeholder="Order #, customer…" className="outline-none text-sm flex-1" />
        </div>
        ...
        <button onClick={() => { setPage(1); load(); }} className="px-4 py-2 border border-gray-200 rounded-lg text-sm hover:bg-gray-50">Filter</button>
            ...
            <tbody>
              ...
              {orders.map(o => (
                <tr key={o.id} className="border-t border-gray-50 hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => openDetail(o.id)} className="flex items-center gap-1 px-3 py-1.5 border border-gray-200 rounded-lg text-xs hover:bg-gray-50 transition-colors">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl my-4">
            ...
            <div className="px-6 py-5 space-y-5 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-5">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${BADGE[detail.status] ?? ""}`}>{detail.status}</span>
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${BADGE[detail.payment_status] ?? ""}`}>{detail.payment_status}</span>
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-500">{detail.ship_line1}{detail.ship_line2 ? ", " + detail.ship_line2 : ""}</p>
                  <p className="text-sm text-gray-500">{detail.ship_city}, {detail.ship_state} — {detail.ship_pincode}</p>
              ...
              {/* Items table */}
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold mb-3">Order Items</p>
                <table className="w-full text-sm">
                  <thead><tr className="bg-gray-50"><th className="px-3 py-2 text-left text-[10px] font-semibold text-gray-400 uppercase">Product</th><th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-400 uppercase">Qty</th><th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-400 uppercase">Unit</th><th className="px-3 py-2 text-right text-[10px] font-semibold text-gray-400 uppercase">Total</th></tr></thead>
                  <tbody>
              ...
              <div className="border-t border-gray-100 pt-4 text-right space-y-1">
                ...
                {detail.discount_amount > 0 && <p className="text-sm text-green-600">Discount: −₹{fmt(detail.discount_amount)}</p>}
                {detail.shipping_charge > 0 && <p className="text-sm text-gray-500">Shipping: ₹{fmt(detail.shipping_charge)}</p>}
